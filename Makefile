...
OCAMLLIBS?=-I src
COQLIBS?=-I src -R theories TESTLIB
COQDOCLIBS?=-R theories TESTLIB
...
install-natdynlink:
	install -d $(DSTROOT)$(COQLIBINSTALL)/TESTLIB; \
	for i in $(CMXSFILESINC); do \
	 install -m 0644 $$i $(DSTROOT)$(COQLIBINSTALL)/TESTLIB/`basename $$i`; \
	done

install:$(if $(HASNATDYNLINK_OR_EMPTY),install-natdynlink)
	cd "theories"; for i in $(VOFILES1); do \
	 install -d "`dirname "$(DSTROOT)"$(COQLIBINSTALL)/TESTLIB/$$i`"; \
	 install -m 0644 $$i "$(DSTROOT)"$(COQLIBINSTALL)/TESTLIB/$$i; \
	done
	install -d $(DSTROOT)$(COQLIBINSTALL)/TESTLIB; \
...
install-doc:
	install -d "$(DSTROOT)"$(COQDOCINSTALL)/TESTLIB/html
	for i in html/*; do \
	 install -m 0644 $$i "$(DSTROOT)"$(COQDOCINSTALL)/TESTLIB/$$i;\
	done
